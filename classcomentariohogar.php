...
class comentariohogar
{ var $id_comentariohogar; 
  var $comen; 
  var $id_exp;    

  function comentariohogar($val1='') // declara el constructor, si trae el id de ficha la busca , si no, trae todas las fichas
  {
    if ($val1!='')
    {
      $obj_Chv=new Conexion();
      $result=$obj_Chv->consulta("select * from comentario_hogar where id_comentariohogar = '$val1'"); // ejecuta la consulta para traer la ficha
      if($obj_Chv->num_rows()!=0){
        $row=  pg_fetch_array($result);
        $this->id_comentariohogar=$row['id_comentariohogar']; 
        $this->comen=$row['comen']; 
  ...
  function getcomentariohogar($val1) // este metodo podria no estar en esta clase, se incluye para simplificar el codigo, lo que hace es traer todas las fichas
  {
    $obj_Chv=new Conexion();
    $query = "select * from comentario_hogar where id_exp = '$val1' order by id_exp ASC";
  ...
  // metodos que devuelven valores
                          function getid_comentariohogar(){ return $this->id_comentariohogar;} 
                          function getcomen(){ return $this->comen;} 
  ...
  // metodos que setean los valores
                          function setid_comentariohogar($val){ return $this->id_comentariohogar=$val;} 
                          function setcomen($val){ return $this->comen=$val;}
                          function setid_exp($val){ return $this->id_exp=$val;} 

  function updatecomentariohogar()  // actualiza la identificacion cargada en los atributos
  {
   $obj_Chv=new Conexion();
   $qverifica="select * from comentario_hogar where id_exp='$this->id_exp'";
   $obj_Chv->consulta($qverifica);
   if($obj_Chv->num_rows()<>0){                          
    $query="update comentario_hogar set comen='$this->comen'"
  ...
  function insertcomentariohogar()  // inserta la identificacion cargada en los atributos
  {
   $obj_Chv=new Conexion();                    
   $qverifica="select * from comentario_hogar where id_exp='$this->id_exp'";
   $obj_Chv->consulta($qverifica);                       
   if($obj_Chv->num_rows()==0){                      
    $query="insert into comentario_hogar( comen,
    id_exp ) 
    values( '$this->comen',
    '$this->id_exp')";                             
                        $obj_Chv->consulta($query); // ejecuta la consulta para traer la identificacion
