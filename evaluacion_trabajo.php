    ...
    <form method="POST" action="evaluacion_trabajo.php?Id=<?php print $id_exp ?>" autocomplete="off"> 
        ...
        <h3 style="text-align: center;"><u></b>Evaluación del caso y Plan de trabajo <b></u></h3>
            ...
            <input type="button" onclick="redirect()" style="width:10%;margin-left:35%;" value="Principal" <?php print $disa ?>>
              <input type="button" onclick="redirect1()" style="width:10%;margin-left:35%;" value="Cancelar" <?php print $disas ?>>
            <input type="submit" style="margin-left: 10%;width:10%" name="submit" id="submitguardar" value ="<?php echo $value ?>">
...
<th></th>';
    ...
    while ($row=  pg_fetch_array($Nuevocomentario)) // recorre los identificaciones uno por uno hasta el fin de la tabla
    {
       print '<tr>'
       .'<td>'.$row['ini'].'</td>'                   
       .'<td><a href="evaluacion_trabajo.php?Id='.$row['id_exp'].'&mdIde='.$row['id_comentario'].'">Modificar</a>&nbsp &nbsp<a href="javascript:;" onclick="redirect()">Principal</a></td>'
...
if (isset($_GET['mio'])) // si la operacion es modificar, este valor viene seteado y ejecuta el siguiente codigo
{
   ...
   -moz-text-align-last: center;"  id="progra"></select></th> 
            <td colspan="3" id="boton"><input type="submit" name="submita" id="submitguardara" value ="<?php echo $value ?>" tabindex="8" required>
...
<th colspan=1></th>';
    ...
    { include './ifprograma.php';
        print '<tr>'
        .'<td>'.$puno.'</td>'
        .'<td><a href="javascript:;" onclick= avisocc("evaluacion_trabajo.php?Id='.$row['id_exp'].'&brIds='.$row['id_prg'].'");>Eliminar</a></td>' 
            //                  .'<td><a href="javascript:;" onclick= avisoi("Salida_Mes.php?brId='.$row['id_salida_mes'].'","'.$row['mes'].'");>Borrar</a></td>'                
            //                  .'<td><a href="javascript:;" onclick= avisoj("Salida_Mes.php?crId='.$row['id_salida_mes'].'","'.$row['mes'].'");>Cerrar Salida</a></td>'                  
        .'</tr>';
    }
...
if (isset($_GET['mio'])) // si la operacion es modificar, este valor viene seteado y ejecuta el siguiente codigo
{
   ...
   <input type="text" style="display: none;width: 50%;" class="ph-center texto" name="otro" id="otro" placeholder="¿Cual?">
        </th>
         <td colspan="3" id="boton"><input type="submit" name="submitare" id="submitguardara" value ="<?php echo $value ?>" tabindex="8"></td>
            </tr>
            </table>
